...
def test_check_notification(client, live_server, measure_memory_usage):
    ...
    notification_form_data = {"notification_urls": notification_url,
                              "notification_title": "New Memento.io Notification - {{watch_url}}",
    ...
    assert bytes("New Memento.io Notification".encode('utf-8')) in res.data
    ...
    assert "New Memento.io Notification - {}".format(test_url) in notification_submission
    ...
    assert b'New Memento.io Notification - ' + test_url.encode('utf-8') in res.data
...
def test_notification_custom_endpoint_and_jinja2(client, live_server, measure_memory_usage):
    ...
    res = client.post(
        ...
        data={
              ...
              "application-notification_title": "New Memento.io Notification - {{ watch_url }} ",
              },
        follow_redirects=True
    )
    assert b'Settings updated' in res.data
...
def _test_color_notifications(client, notification_body_token):
    ...
    from memento.diff import ADDED_STYLE, REMOVED_STYLE
