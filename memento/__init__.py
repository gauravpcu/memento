...
from memento.flask_app import changedetection_app
...
def sigshutdown_handler(_signo, _stack_frame):
    ...
    try:
        from memento.flask_app import update_q, notification_q
        update_q.close()
    ...
    from memento.flask_app import socketio_server
...
def main():
    ...
    if os.name == 'nt':
        datastore_path = os.path.expandvars(r'%APPDATA%\memento')
    ...
    logger.success(f"memento version {get_version()} starting.")
    ...
    try:
        datastore = store.MementoStore(datastore_path=app_config['datastore_path'], version_tag=__version__)
